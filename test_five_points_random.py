import asyncio
import math
import random

from mavsdk import System
from mavsdk.mission import MissionItem, MissionPlan

from mission.flight.preflight_confirm import (
    print_mission_plan,
    print_qgc_clear_reminder,
    require_fly_confirmation,
    verify_disarmed_and_on_ground,
    verify_mission_uploaded,
)

SYSTEM_ADDRESS = "serial:///dev/cu.usbserial-DU0D65S7:57600"

CRUISE_ALTITUDE_M = 25.0
ACCEPTANCE_RADIUS_M = 3.0

RADIUS_M = 10.0
MIN_SPACING_M = 5.0
NUM_WAYPOINTS = 5
RANDOM_SEED = 42
MAX_SAMPLE_ATTEMPTS = 10_000

_EARTH_RADIUS_M = 6_371_000.0


def _offset_latlon(lat_deg: float, lon_deg: float, north_m: float, east_m: float) -> tuple[float, float]:
    d_lat = (north_m / _EARTH_RADIUS_M) * (180.0 / math.pi)
    d_lon = (east_m / (_EARTH_RADIUS_M * math.cos(math.radians(lat_deg)))) * (180.0 / math.pi)
    return lat_deg + d_lat, lon_deg + d_lon


def _generate_offsets() -> list[tuple[float, float]]:
    rng = random.Random(RANDOM_SEED)
    points: list[tuple[float, float]] = []
    attempts = 0

    while len(points) < NUM_WAYPOINTS:
        attempts += 1
        if attempts > MAX_SAMPLE_ATTEMPTS:
            raise RuntimeError(
                f"Could not sample {NUM_WAYPOINTS} points >= {MIN_SPACING_M} m apart "
                f"within a {RADIUS_M} m radius after {MAX_SAMPLE_ATTEMPTS} attempts."
            )

        r = RADIUS_M * math.sqrt(rng.random())
        theta = rng.uniform(0.0, 2 * math.pi)
        north_m = r * math.cos(theta)
        east_m = r * math.sin(theta)

        if any(math.hypot(north_m - n, east_m - e) < MIN_SPACING_M for n, e in points):
            continue

        points.append((north_m, east_m))

    return points


def _validate_offsets(points: list[tuple[float, float]]) -> None:
    assert len(points) == NUM_WAYPOINTS, f"expected {NUM_WAYPOINTS} points, got {len(points)}"

    for n, e in points:
        dist_from_home = math.hypot(n, e)
        assert dist_from_home <= RADIUS_M + 1e-6, (
            f"point ({n:.2f}, {e:.2f}) is {dist_from_home:.2f}m from home, "
            f"exceeds {RADIUS_M}m radius"
        )

    for i in range(len(points)):
        for j in range(i + 1, len(points)):
            n1, e1 = points[i]
            n2, e2 = points[j]
            dist = math.hypot(n1 - n2, e1 - e2)
            assert dist >= MIN_SPACING_M - 1e-6, (
                f"points {i + 1} and {j + 1} are only {dist:.2f}m apart, "
                f"minimum is {MIN_SPACING_M}m"
            )


async def main() -> None:
    random.seed(RANDOM_SEED)

    drone = System()

    print(f"Connecting to Pixhawk on {SYSTEM_ADDRESS} ...")
    await drone.connect(system_address=SYSTEM_ADDRESS)

    print("Waiting for connection...")
    async for state in drone.core.connection_state():
        if state.is_connected:
            print("Connected to drone.")
            break

    print("Waiting for global position and home position lock...")
    async for health in drone.telemetry.health():
        if health.is_global_position_ok and health.is_home_position_ok:
            print("GPS and home position OK.")
            break

    print("Fetching home position...")
    async for home in drone.telemetry.home():
        home_lat, home_lon = home.latitude_deg, home.longitude_deg
        break
    print(f"Home: lat={home_lat:.7f}, lon={home_lon:.7f}")

    offsets = _generate_offsets()
    _validate_offsets(offsets)

    print(f"Waypoints (random, seed={RANDOM_SEED}, radius={RADIUS_M}m, min spacing={MIN_SPACING_M}m):")
    waypoints = []
    for i, (north_m, east_m) in enumerate(offsets, start=1):
        lat, lon = _offset_latlon(home_lat, home_lon, north_m, east_m)
        waypoints.append((lat, lon))
        print(f"  {i}: north={north_m:+.2f}m east={east_m:+.2f}m -> lat={lat:.7f}, lon={lon:.7f}")

    mission_items = []
    for lat, lon in waypoints:
        mission_items.append(
            MissionItem(
                lat,
                lon,
                CRUISE_ALTITUDE_M,
                5.0,
                True,
                float("nan"),
                float("nan"),
                MissionItem.CameraAction.NONE,
                float("nan"),
                float("nan"),
                ACCEPTANCE_RADIUS_M,
                float("nan"),
                float("nan"),
                MissionItem.VehicleAction.NONE,
            )
        )

    print_mission_plan(mission_items)

    print("Uploading mission plan (5 waypoints)...")
    await drone.mission.upload_mission(MissionPlan(mission_items))
    await verify_mission_uploaded(drone, mission_items)

    await verify_disarmed_and_on_ground(drone)
    print_qgc_clear_reminder()
    require_fly_confirmation()

    print("Arming...")
    await drone.action.arm()

    print("Starting mission...")
    await drone.mission.start_mission()

    total_waypoints = len(mission_items)
    last_reported = -1

    async def report_progress() -> None:
        nonlocal last_reported
        async for progress in drone.mission.mission_progress():
            if progress.current != last_reported:
                last_reported = progress.current
                print(f"Waypoint {progress.current}/{total_waypoints}")
            if progress.current >= total_waypoints:
                return

    async def report_position() -> None:
        async for position in drone.telemetry.position():
            print(
                f"  pos: lat={position.latitude_deg:.7f}, "
                f"lon={position.longitude_deg:.7f}, "
                f"alt={position.relative_altitude_m:.1f}m"
            )

    position_task = asyncio.ensure_future(report_position())
    try:
        await report_progress()
    finally:
        position_task.cancel()

    print("All 5 waypoints complete.")
    print("Mission finished. Auto-landing disabled -- take manual control to land.")


if __name__ == "__main__":
    asyncio.run(main())
